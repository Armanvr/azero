...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	const user = await UserModel.findOne({ id: session.userId }).lean<User>()
	...
	const [summary, media, equipment, currencies] = await Promise.all([
		...
		bnetFetch(
			`${EU_BASE}/profile/wow/character/${realmSlug}/${name}/character-media?${profileNs}&locale=${locale}`,
			token,
		),
		bnetFetch(
			`${EU_BASE}/profile/wow/character/${realmSlug}/${name}/equipment?${profileNs}&locale=${locale}`,
			token,
		),
		bnetFetch(
			`${EU_BASE}/profile/wow/character/${realmSlug}/${name}/currencies?${profileNs}&locale=${locale}`,
			token,
		),
	])
	...
	if (currencies?.currencies) {
		const goldEntry = currencies.currencies.find(
			(c: { currency: { id: number }; quantity: number }) => c.currency?.id === 1,
		)
	...
	await CharacterModel.updateOne({ _id: existing._id }, { $set: enriched })
